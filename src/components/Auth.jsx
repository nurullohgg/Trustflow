...
import { COLORS, ShieldIcon, CheckCircleIcon } from './Shared';
...
export function AuthChooser({ onClose, onPick }) {
  const opts = [
    { id:'seeker',  icon:'👤', title:'Ish qidiruvchi',          sub:'Job seeker / Open to work',  color:COLORS.teal  },
    { id:'company', icon:'🏢', title:'Kompaniya / Ish beruvchi', sub:'Company / Recruiter',         color:COLORS.amber },
    { id:'gov',     icon:'🛡', title:'Davlat nazorat',           sub:'Government admin (login only)', color:COLORS.red },
  ];
  return (
    <AuthModalShell title="Qaysi rolda kirasiz?" subtitle="Choose your role" onClose={onClose} width={440}>
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {opts.map(o=>(
          <button key={o.id} onClick={()=>onPick(o.id)} style={{
            display:'flex', alignItems:'center', gap:14, padding:'14px 16px',
            border:`1.5px solid ${COLORS.border}`, borderRadius:12,
            background:COLORS.white, cursor:'pointer', fontFamily:"'Inter',sans-serif",
            textAlign:'left', transition:'all .15s',
          }}
          onMouseEnter={e=>{e.currentTarget.style.borderColor=o.color;e.currentTarget.style.background=COLORS.bgAlt;}}
          onMouseLeave={e=>{e.currentTarget.style.borderColor=COLORS.border;e.currentTarget.style.background=COLORS.white;}}>
            <div style={{
              fontSize:24, width:42, height:42, borderRadius:10, background:COLORS.tealBg,
              display:'flex', alignItems:'center', justifyContent:'center',
            }}>{o.icon}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, fontWeight:700, color:COLORS.text }}>{o.title}</div>
              <div style={{ fontSize:12, color:COLORS.textMuted, marginTop:2 }}>{o.sub}</div>
            </div>
...
export function JobSeekerAuth({ onClose, onSuccess }) {
...
export function CompanyAuth({ onClose }) {
...
export function GovAuth({ onClose, onSuccess }) {
