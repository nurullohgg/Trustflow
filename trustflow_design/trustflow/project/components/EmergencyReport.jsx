// EmergencyReport.jsx — Emergency corruption report with GPS, voice/video recording

function EmergencyReportModal({ onClose }) {
  const [step, setStep] = React.useState('verify'); // verify, form, recording, done
  const [description, setDescription] = React.useState('');
  const [location, setLocation] = React.useState('');
  const [recordingType, setRecordingType] = React.useState(null); // null, 'audio', 'video'
      ...
      <div style={{
        ...
        {step === 'form' && (
          <>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                {[
                  { id:'audio', label:'🎙 Ovoz yozib olish', icon:MicIcon },
                  { id:'video', label:'📹 Video yozib olish', icon:VideoIcon },
                ].map(opt => (
        ...
        {step === 'recording' && (
          <>
            <div style={{ textAlign:'center', marginBottom:32 }}>
              <div style={{
                width:80, height:80, borderRadius:'50%', background: isRecording ? COLORS.redBg : COLORS.tealBg,
                display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px',
              }}>
            ...
            <button onClick={() => setIsRecording(!isRecording)}
              ...
              {isRecording ? (
                <>
                  <span style={{ fontSize:20, animation:'blink 1s infinite' }}>●</span>
                  Ushlab turish va gapirish (Drag to record)
                </>
              ) : (
                <>
...
Object.assign(window, { EmergencyReportModal });
